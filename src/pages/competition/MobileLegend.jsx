...
import { Success, Error, MobileLegendImage } from '../../assets/images/layout'
...
const MobileLegend = () => {
  ...
  // Get Data API
  useEffect(() => {
    Axios.get('https://itfest.my.id/api/mobile-legend').then(res => setData(res.data.data))
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await Axios.post('https://itfest.my.id/api/mobile-legend', postData, {
        headers: {
  ...
  return (
    <div className='bg-gradient-to-tl flex-col  from-gray-900 via-gray-900 to-gray-700 w-full justify-center min-h-screen flex items-center'>
      <Helmet>
        <meta name="description" content="Kompetisi Mobile Legends" />
        <meta name="keywords" content="Kompetisi Mobile Legends" />
        <title>Kompetisi Mobile Legends</title>
        <link rel="icon" href={Logo} />
      </Helmet>
      <div className="min-h-screen items-center justify-center flex flex-col sm:flex-row">
        <img src={MobileLegendImage} alt="" className='sm:w-1/2 m-auto' />
        <div className="mb-28 -mt-24 sm:mb-0 sm:-mt-0 text-center sm:px-14">
          <p className="zaptron text-center sm:text-5xl text-4xl mt-4 text-white animate-pulse">Mobile Legends</p>
          <p className="comfortaa text-xs sm:text-base text-gray-500 text-center px-4 sm:px-10"><i>"Cemas kau dek, bek bek bek bek"</i></p>
          <p className="mt-10 text-gray-200 comfortaa text-center mb-5">HTM Kompetisi Mobile Legends : 45k</p>
          <a href="https://drive.google.com/drive/folders/1cqVO5oLHMQJotlR0rxnNZhxxgf3k85hw" target="blank" className='py-2 px-10 bg-gray-400 rounded-lg hover:bg-gray-300'>Guide Book</a>
        </div>
      </div>
      <p data-aos="fade-up" data-aos-duration="1000" className='text-white zaptron text-sm sm:text-lg border-b border-gray-200 mt-10 py-2'>Form Pendaftaran Lomba Mobile Legends</p>
      ...
      <p className='text-xs sm:text-base pb-3 text-white'>ada pertanyaan? <a target="blank" className='text-blue-500 underline' href="https://wa.me/6285322488609?text=Halo%20kak,%20Saya%20mau%20bertanya%20perihal%20kompetisi%20Mobile%20Legends%20IT%20FESTIVAL%202022">Klik disini untuk menghubungi admin</a></p>
          ...
          <Modal.Body>
            <div className="text-center">
              ...
              <h3 className="mb-5 text-sm font-normal text-red-600 comfortaa">
                Yaahh, Kantong Erco ga muat. Kecilin file kamu lalu kirim lagi ke Erco, Yaa!
              </h3>
              <div className="flex justify-center gap-4">
                <Button
                  color="failure"
